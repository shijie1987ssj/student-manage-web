...
<script type="text/javascript">
	var listUrl = '${ctx}/subjects/subjectsListData.do'; 
	$(function(){
			$('#dataListDiv').datagrid({
				...
				idField:'id',
				columns:[[
							{field:'subjectname',title:'课程名称',align : 'center',width:50}, 
							{field:'clubname',title:'院系名称',align : 'center',width:50}, 
						{field:'op',title:'操作',align : 'center',width:50,
								formatter:function(value,rec){
									var txt2 = "<a href='javascript:void(0)' onclick='editData(\""+rec.subjectname+"\");'>修改</a>";
									var txt3 = "<a href='javascript:void(0)' onclick='delData(\""+rec.subjectname+"\");'>删除</a>";
									return txt2 + " | " + txt3;
								}
							}
				]],
	...
	function addData(){
		var url = '${ctx}/subjects/toAdd.do';
		showDialog('添加信息',url,'moresmall',true);
	}
	function editData(id){
		if(id!=null){
			var url = '${ctx}/subjects/toEdit.do?subjectname='+id;
			showDialog('信息修改',url,'moresmall',true);
		}else{
			var node = getSelected();
			if(node){
				var url = '${ctx}/subjects/toEdit.do?subjectname='+node.subjectname;
				showDialog('信息修改',url,'moresmall',true);
			}
		}
	}
	function delData(id){
		...
		if(id!=null){
			...
			var node = getSelected();
			if(node){
				delId = node.subjectname;
			}
		}
		$.messager.confirm("操作提示", "您确定要删除吗？", function (data) { 
			if (data) {  
				var url ="${ctx}/subjects/subjectsDelete.do?subjectname="+delId;
	...
	function doQuery() {
			...
			queryParams.clubname = $('#clubname').val();
...
<div title="查询条件" id="sellerUnitList" class="easyui-panel"
	...
	<form  id="searchform" name="searchform" method="post">		
		<table class="datagrid-body" width="100%">
			<tr>
							<td class="datagrid-header">课程名称：</td>
							<td><input type="text" class="easyui-textbox" id="subjectname" name="subjectname" /></td>
							<td class="datagrid-header">院系名称：</td>
							<td><input type="text" class="easyui-textbox" id="clubname" name="clubname" /></td>
			</tr>
			<tr>
