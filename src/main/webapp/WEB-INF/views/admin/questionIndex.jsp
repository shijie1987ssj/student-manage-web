...
<script type="text/javascript">
	var listUrl = '${ctx}/question/questionListData.do'; 
	$(function(){
			$('#dataListDiv').datagrid({
				...
				idField:'id',
				columns:[[
							{field:'questionid',title:'评价问题题号',align : 'center',width:50}, 
							{field:'questionscore',title:'评价问题分数',align : 'center',width:50}, 
							{field:'question',title:'评价问题',align : 'center',width:50}, 
						{field:'op',title:'操作',align : 'center',width:50,
								formatter:function(value,rec){
									var txt2 = "<a href='javascript:void(0)' onclick='editData(\""+rec.questionid+"\");'>修改</a>";
									var txt3 = "<a href='javascript:void(0)' onclick='delData(\""+rec.questionid+"\");'>删除</a>";
									return txt2 + " | " + txt3;
								}
							}
				]],
	...
	function addData(){
		var url = '${ctx}/question/toAdd.do';
		showDialog('添加审批数据字典信息',url,'moresmall',true);
	}
	function editData(id){
		if(id!=null){
			var url = '${ctx}/question/toEdit.do?questionid='+id;
			showDialog('审批数据字典信息修改',url,'moresmall',true);
		}else{
			var node = getSelected();
			if(node){
				var url = '${ctx}/question/toEdit.do?questionid='+node.questionid;
				showDialog('审批数据字典信息修改',url,'moresmall',true);
			}
		}
	}
	function delData(id){
		...
		if(id!=null){
			...
			var node = getSelected();
			if(node){
				delId = node.questionid;
			}
		}
		$.messager.confirm("操作提示", "您确定要删除吗？", function (data) { 
			if (data) {  
				var url ="${ctx}/question/questionDelete.questionid?id="+delId;
	...
	function doQuery() {
			...
			queryParams.questionid = $('#questionid').val(); 
...
<div title="查询条件" id="sellerUnitList" class="easyui-panel"
	...
	<form  id="searchform" name="searchform" method="post">		
		<table class="datagrid-body" width="100%">
			<tr>
							<td class="datagrid-header">评价问题题号：</td>
							<td><input type="text" class="easyui-textbox" id="questionid" name="questionid" /></td> 
			</tr>
			<tr>
