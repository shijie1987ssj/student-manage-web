...
<script type="text/javascript">
	var listUrl = '${ctx}/club/clubListData.do'; 
	$(function(){
				...
				columns:[[
						...
						{field:'op',title:'操作',align : 'center',width:50,
								formatter:function(value,rec){
									var txt2 = "<a href='javascript:void(0)' onclick='editData(\""+rec.clubname+"\");'>修改</a>";
									var txt3 = "<a href='javascript:void(0)' onclick='delData(\""+rec.clubname+"\");'>删除</a>";
									return txt2 + " | " + txt3;
								}
							}
				]],
	...
	function addData(){
		var url = '${ctx}/club/toAdd.do';
		showDialog('添加信息',url,'moresmall',true);
	}
	function editData(id){
		if(id!=null){
			var url = '${ctx}/club/toEdit.do?clubname='+id;
			showDialog('信息修改',url,'moresmall',true);
		}else{
			var node = getSelected();
			if(node){
				var url = '${ctx}/club/toEdit.do?clubname='+node.clubname;
				showDialog('信息修改',url,'moresmall',true);
			}
		}
	}
	function delData(id){
		...
		$.messager.confirm("操作提示", "您确定要删除吗？", function (data) { 
			if (data) {  
				var url ="${ctx}/club/clubDelete.do?id="+delId;
