...
<script type="text/javascript">
	var listUrl = '${ctx}/teacher/teacherListData.do'; 
	$(function(){
			$('#dataListDiv').datagrid({
				...
				columns:[[
							...
							{field:'pingjia',title:'是否评价',align : 'center',width:50}, 
						{field:'op',title:'操作',align : 'center',width:50,
								formatter:function(value,rec){
									var txt2 = "<a href='javascript:void(0)' onclick='editData(\""+rec.teacherid+"\");'>修改</a>";
									var txt3 = "<a href='javascript:void(0)' onclick='delData(\""+rec.teacherid+"\");'>删除</a>";
									return txt2 + " | " + txt3;
								}
							}
				]],
	...
	function addData(){
		var url = '${ctx}/teacher/toAdd.do';
		showDialog('添加信息',url,'moremiddle',true);
	}
	function editData(id){
		if(id!=null){
			var url = '${ctx}/teacher/toEdit.do?teacherid='+id;
			showDialog('信息修改',url,'moremiddle',true);
		}else{
			var node = getSelected();
			if(node){
				var url = '${ctx}/teacher/toEdit.do?teacherid='+node.teacherid;
				showDialog('信息修改',url,'moremiddle',true);
			}
		}
	}
	function delData(id){
		...
		$.messager.confirm("操作提示", "您确定要删除吗？", function (data) { 
			if (data) {  
				var url ="${ctx}/teacher/teacherDelete.do?teacherid="+delId;
