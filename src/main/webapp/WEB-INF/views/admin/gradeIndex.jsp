...
<script type="text/javascript">
	var listUrl = '${ctx}/grade/gradeListData.do'; 
	$(function(){
			$('#dataListDiv').datagrid({
				...
				idField:'id',
				columns:[[
							{field:'gradename',title:'年级',align : 'center',width:50}, 
						{field:'op',title:'操作',align : 'center',width:50,
								formatter:function(value,rec){
									var txt2 = "<a href='javascript:void(0)' onclick='editData(\""+rec.gradename+"\");'>修改</a>";
									var txt3 = "<a href='javascript:void(0)' onclick='delData(\""+rec.gradename+"\");'>删除</a>";
									return txt2 + " | " + txt3;
								}
							}
				]],
	...
	function addData(){
		var url = '${ctx}/grade/toAdd.do';
		showDialog('添加信息',url,'moresmall',true);
	}
	function editData(id){
		if(id!=null){
			var url = '${ctx}/grade/toEdit.do?gradename='+id;
			showDialog('信息修改',url,'moresmall',true);
		}else{
			var node = getSelected();
			if(node){
				var url = '${ctx}/grade/toEdit.do?gradename='+node.gradename;
				showDialog('信息修改',url,'moresmall',true);
			}
		}
	}
	function delData(id){
		...
		if(id!=null){
			...
			var node = getSelected();
			if(node){
				delId = node.gradename;
			}
		}
		$.messager.confirm("操作提示", "您确定要删除吗？", function (data) { 
			if (data) {  
				var url ="${ctx}/grade/gradeDelete.do?gradename="+delId;
	...
	function doQuery() {
			...
			queryParams.gradename = $('#gradename').val();
...
<div title="查询条件" id="sellerUnitList" class="easyui-panel"
	...
	<form  id="searchform" name="searchform" method="post">		
		<table class="datagrid-body" width="100%">
			<tr>
							<td class="datagrid-header">年级：</td>
							<td><input type="text" class="easyui-textbox" id="gradename" name="gradename" /></td>
			</tr>
			<tr>
