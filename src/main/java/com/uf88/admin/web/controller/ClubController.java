...
public class ClubController {
	...
	private static final String VIEW_INDEX = "clubIndex";
	private static final String AJAX_LISTDATA = "clubListData";
	private static final String VIEW_TOADD = "toAdd";
	private static final String AJAX_ADD = "/clubAdd";
	private static final String VIEW_TODETAIL = "/clubDetail";
	private static final String VIEW_TOEDIT = "toEdit";
	private static final String AJAX_EDIT = "/clubEdit";
	private static final String AJAX_DELETE = "clubDelete";
	...
	@RequestMapping(value = { VIEW_INDEX }, produces = { HTML_UTF8 }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public ModelAndView clubIndex(HttpServletRequest request) {
		ModelAndView mv = new ModelAndView("admin/clubIndex");
		return mv;
	}
	...
	@RequestMapping(value = { AJAX_LISTDATA }, method = { RequestMethod.GET, RequestMethod.POST }, produces = {
			JSON_UTF8 })
	@ResponseBody
	public Object clubListData(HttpServletRequest request, @ModelAttribute ClubQuery query,
		...
		if (query == null)
			query = new ClubQuery();
		PageQuery<ClubQuery> pageQuery = new PageQuery<ClubQuery>();
		pageQuery.setPageNo(pageNo);
		pageQuery.setPageSize(pageSize);
		pageQuery.setQuery(query);

		PageListResult<Club> info = clubService.searchPageClub(pageQuery);
	...
	public ModelAndView clubDetail(@RequestParam(required = true, value = "clubname") String clubname) {
		ModelAndView mav = new ModelAndView();
		mav.setViewName("admin/clubDetail");
		GenericResult<Club> result = clubService.findByPriKey(clubname);
		if (result != null)
			mav.addObject("club", result.getValue());
		else
	...
	@ResponseBody
	public Object clubAdd(@ModelAttribute Club club) {
		BasicResult resultMap = new BasicResult();
		GenericResult<String> result = clubService.addClub(club);
	...
	public ModelAndView toEdit(@RequestParam(required = true, value = "clubname") String clubname) {
		ModelAndView mav = new ModelAndView();
		mav.setViewName("admin/clubEdit");
		GenericResult<Club> result = clubService.findByPriKey(clubname);
		mav.addObject("club", result.getValue());
		return mav;
	}
	...
	@ResponseBody
	public Object clubEdit(@ModelAttribute Club club) {
		BasicResult resultMap = new BasicResult();
		BasicResult result = clubService.modifyClub(club);
	...
	@ResponseBody
	public Object clubDelete(@RequestParam(required = true, value = "clubname") String clubname) {
		BasicResult resultMap = new BasicResult();
		BasicResult result = clubService.deleteByPriKey(clubname);
		if (ResultUtil.isSuccess(result)) {
			resultMap.setCode(ResultCode.SUCCESS.getCode());
			resultMap.setMessage("操作成功");
		} else {
